...
# start jupyter 
nohup jupyter ${data.coder_parameter.jupyter.value} --${local.jupyter-type-arg}App.token="" --ip="*" >/dev/null 2>&1 &

# add additional Python libraries
# Install libraries in the background and suppress all output
pip3 install --user pandas numpy sympy pydantic jax jaxlib scipy graphviz pyscf >/dev/null 2>&1 &

# clone repo example (this is private repo so commenting out)
...
SERVICE_URL=https://open-vsx.org/vscode/gallery ITEM_URL=https://open-vsx.org/vscode/item code-server --install-extension alefragnani.Bookmarks
SERVICE_URL=https://open-vsx.org/vscode/gallery ITEM_URL=https://open-vsx.org/vscode/item code-server --install-extension donjayamanne.githistory
